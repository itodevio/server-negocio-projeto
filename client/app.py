...
async def main():
    async with aiohttp.ClientSession() as session:
        ret = await asyncio.gather(*[doGetBalance(session) for account in accounts])
    async with aiohttp.ClientSession() as session:
        ret = await asyncio.gather(*[doDeposit(session) for account in accounts])
